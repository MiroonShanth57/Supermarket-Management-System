...
import dao.Customer.OrderDetailsDAO;
...
public class OrderBOImpl implements OrderBO {
    ...
    @Override
    public ObservableList<OrderManageTM> getDataForTable(String orderId) throws SQLException, ClassNotFoundException {
        ObservableList<OrderManageTM> obs = FXCollections.observableArrayList();
        ObservableList<OrderManageTM> dataForManageOrder = orderDetailsDAO.getDataForManageOrder(orderId);

        for (OrderManageTM tm :
                dataForManageOrder) {
            Item item = getItemDescriptionAndUnitPrice(tm.getItemCode());
            obs.add(new OrderManageTM(
                    tm.getItemCode(),
                    item.getDescription(),
                    tm.getOrderQty(),
                    item.getUnitPrice()
